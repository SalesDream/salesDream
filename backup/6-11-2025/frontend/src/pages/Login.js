...
const [loading, setLoading] = useState(false);
  ...
  const submit = async (e) => {
    ...
    try {
      setLoading(true);
      // Call backend endpoint that validates email+password and sends OTP or returns bypass
      const { data } = await api.post("/api/auth/login-request", { email, password });

      // If server allows bypass (for allowlisted email), server returns bypass & token
      if (data?.bypass) {
      ...
      // Otherwise OTP has been sent — move to OTP phase
      setPhase(2);
