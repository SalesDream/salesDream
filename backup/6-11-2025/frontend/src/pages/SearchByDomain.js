...
/**
 * SearchByDomain page (updated to match SearchByEmail-style filters)
 *
 * - Top input is domain (not phone).
 ...
 * - Syncs the top domain input to filters.domain when used so chips show it
 * - State filter uses a static list of US state codes (not pulled from backend)
 * - City / Company / Website / Employees / SalesVolume fields made multi-select based on page facets
 */
...
function MultiSelect({ label, options, values, onChange, searchable = true }) {
  ...
  const toggle = (val) => {
    let cur = Array.isArray(values) ? values : [];
...
const DateCell = (p) => {
  ...
  if (!v) return <span>--</span>;
  const d = new Date(v);
  if (isNaN(d)) return <span>{String(v)}</span>;
...
/* ======================
   MAIN SearchByDomain Component
   ====================== */
export default function SearchByDomain() {
  ...
  // Filters shape (updated to use arrays where appropriate)
  ...
  // Facets from current page results (lightweight, similar to SearchByEmail)
  ...
  // CONTACT: show full values and add copy buttons (phone + email)
  ...
  // Extra fields: removed id, added first_name & last_name
  const ALL_FIELDS = [
    "name", // full name
    "first_name",
    "last_name",
  ...
  /* ---------- normalize a backend row to UI row (aligned with SearchByEmail) ---------- */
  ...
  const fetchPage = async (pageNumber = 0, overrides = {}) => {
    ...
    try {
      ...
      // If user typed domain in top box, prefer that (sync f too so chips show it)
      if (domainInput && domainInput.trim() !== "") {
        // normalize domain: remove protocol and www
        let d = String(domainInput).trim().toLowerCase();
        d = d.replace(/^https?:\/\//, "").replace(/^www\./, "").replace(/\/.*$/, "");
        // set as an array so buildParamsFromFilters will comma-join (multi-select style)
        mergedFilters.domain = [d];
        setF((s) => ({ ...s, domain: [d] }));
      }
  ...
  /* ---------- Search handlers & helpers ---------- */
  // DOMAIN search disabled if input empty
  const isDomainSearchDisabled = useMemo(() => {
    return String(domainInput || "").trim() === "";
  }, [domainInput]);
  ...
  const handleDomainSearch = async () => {
    if (isDomainSearchDisabled) return;
    const v = (domainInput || "").trim();
    if (!v) return;
    // normalize domain for override (no protocol, no path)
    let nv = v.toLowerCase();
    nv = nv.replace(/^https?:\/\//, "").replace(/^www\./, "").replace(/\/.*$/, "");
    // pass as array override so it's sent as domain=nv (CSV)
    await fetchPage(0, { domain: [nv] });
  };
  ...
  // Active chips (domain + others) — now handles arrays consistently
    ...
    <div className="h-[calc(96vh-0px)] flex">
      ...
      <main className="flex-1 px-2 pt-0 pb-6 -mt-3">
        <div className="mb-1 flex items-center justify-between">
          <h1 className="text-xl font-semibold">Search by Domain</h1>
          <div className="flex items-center gap-2">
            {/* domain input + exact checkbox + Search button */}
            <div className="ml-3 flex items-center gap-2">
              <input
                ...
                placeholder="Enter domain or partial (e.g. wells fargo, wellsfargo.com)"
              ...
              <button
                ...
                disabled={isDomainSearchDisabled}
