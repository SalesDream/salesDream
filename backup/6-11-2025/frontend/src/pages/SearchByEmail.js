...
// src/pages/SearchByEmail.jsx (updated)
// Changes made:
// 1. Removed 'id' column from ALL_FIELDS
// 2. Renamed filter label "Contact Name" -> "Full Name"
// 3. Contact column now shows actual phone/email values and includes copy buttons
// 4. Added first_name and last_name columns, derived from full name (basic split)
// 5. Blank/null values show '--' across the grid (defaultColDef valueFormatter)
// 6. Kept original behavior otherwise and styling similar to Dashboard reference
...
/* ---------- Small helpers & UI blocks (kept & slightly adjusted) ---------- */
...
/* ======================
   MAIN SearchByEmail Component (updated)
   ====================== */
export default function SearchByEmail() {
  ...
  const baseColumns = useMemo(
    () => [
      ...
      {
        // display full name in People column (header label shown as "Full Name" via extras but keeping People for compact UI)
        headerName: "People",
        field: "contact_name",
        flex: 2,
        minWidth: 360,
        ...
        <aside className="relative h-full bg-white border rounded-xl shadow-md overflow-visible">
          ...
          <div className="px-3 pb-3 pt-3 overflow-y-auto overflow-x-hidden h-[calc(100%-40px)] space-y-2">
            <FilterSection icon={User} label="Contact">
              {/* Changed label to Full Name per request */}
