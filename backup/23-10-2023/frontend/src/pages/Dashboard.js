...
/**
 * NOTE:
 * - Filters rail updated to derive options from row.__raw (merged/linked)
 * - Years Of Experience section removed
 * - Company / Domain and Website / Domain options adjusted to use specific merged/linked keys
 * - Employee Count & Total Revenue changed to dropdowns populated from merged.NumEmployees and merged.SalesVolume
 * - STATE options are now static (hard-coded US state codes) as requested
 */
        ...
        <aside className="relative h-full bg-white border rounded-xl shadow-md overflow-visible">
          ...
          <div className="px-3 pb-3 pt-3 overflow-y-auto overflow-x-hidden h-[calc(100%-40px)] space-y-2">
            ...
            <FilterSection icon={MapPin} label="Location">
              <MultiSelect
                label="State"
                options={US_STATES} /* static list requested */
                values={f.state_code}
...
export default function Dashboard() {
  ...
  const buildQuery = (filters) => {
    ...
    set("company_name", filters.company_name && filters.company_name.join(","));
    set("city", filters.city && filters.city.join(","));
    set("zip_code", filters.zip_code);
    set("website", filters.website && filters.website.join(","));
    set("contact_full_name", filters.contact_full_name);

    set("state_code", filters.state_code && filters.state_code.join(","));
    set("company_location_country", filters.company_location_country && filters.company_location_country.join(","));
    set("industry", filters.industry && filters.industry.join(","));
    set("job_title", filters.job_title && filters.job_title.join(","));
    set("contact_gender", filters.contact_gender && filters.contact_gender.join(","));
    set("skills", filters.skills_tokens && filters.skills_tokens.join(","));
    ...
    set("employees", filters.employees && filters.employees.join(","));
    set("sales_volume", filters.sales_volume && filters.sales_volume.join(","));

    // quick keys (phone, email, domain)
    set("phone", filters.phone);
    set("normalized_email", filters.normalized_email);
    set("domain", filters.domain && filters.domain.join(","));
