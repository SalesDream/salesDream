...
export default function SearchByDomain() {
  ...
  const numericRight = new Set(["employees", "sales_volume"]);

  const extraColumns = useMemo(() => {
    const exclude = new Set(baseColumns.map((c) => c.field).filter(Boolean));
    return ALL_FIELDS.filter((fld) => !exclude.has(fld)).reduce((acc, field) => {
      const def = {
      ...
      if (["website", "domain", "linkedin_url"].includes(field)) {
  ...
  }, [baseColumns, phoneOnlyRenderer, emailOnlyRenderer]);

  const columnDefs = useMemo(() => {
    const result = [...baseColumns];
    const added = new Set(result.map((c) => c.field).filter(Boolean));

    for (const fName of ALL_FIELDS) {
      if (added.has(fName)) continue;
      if (extraColumns[fName]) {
        result.push(extraColumns[fName]);
        added.add(fName);
      }
    }
    return result;
  ...
  const normalizeRow = (raw = {}) => {
    ...
    const employees = raw.employees || pick("merged.NumEmployees") || "";
    const sales_volume = raw.sales_volume || pick("merged.SalesVolume") || "";
  ...
  const fetchPage = async (pageNumber = 0, overrides = {}) => {
    ...
    try {
      ...
      if (domainInput && domainInput.trim() !== "") {
        mergedFilters.domain = domainInput.trim();
        setF((s) => ({ ...s, domain: domainInput.trim() }));
      }
  ...
  const activeChips = useMemo(() => {
    ...
    if (f.contact_full_name) push(`Name: ${f.contact_full_name}`, () => setF((s) => ({ ...s, contact_full_name: "" })));
    if (f.job_title) push(`Title: ${f.job_title}`, () => setF((s) => ({ ...s, job_title: "" })));
    if (f.skills_tokens) push(`Skills: ${f.skills_tokens}`, () => setF((s) => ({ ...s, skills_tokens: "" })));

    if (f.employees) push(`Employees: ${f.employees}`, () => setF((s) => ({ ...s, employees: "" })));
    if (f.sales_volume) push(`Revenue: ${f.sales_volume}`, () => setF((s) => ({ ...s, sales_volume: "" })));
    ...
    if (f.q) push(`Global: ${f.q}`, () => setF((s) => ({ ...s, q: "" })));
      ...
      <div className="relative z-20 shrink-0 w-[250px] px-3">
        ...
        <div className="top-4 z-30 pb-3">
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-3">
            <label className="sr-only">Search by Domain</label>
        ...
        <aside
          ...
          style={{ height: "calc(100vh - 110px)", padding: 12 }}
        >
          ...
          <div className="px-2 pb-3 pt-3 space-y-3">
            ...
            <FilterSection icon={User} label="Contact">
              ...
              />
              <TextInput
                label="Email (normalized_email)"
                value={f.normalized_email}
                onChange={(v) => setF((s) => ({ ...s, normalized_email: v }))}
              ...
              <TextInput
                ...
                placeholder="digits or formatted"
              />
            </FilterSection>
