 ...
 * - Size & Revenue: Employee Count and Total Revenue are multi-select lists derived from merged.NumEmployees and merged.SalesVolume
 * - Years Of Experience section removed
 *
 * All filters that are arrays are sent to backend as comma-separated values.
 */
...
export default function SearchByPhone() {
  ...
  // Facets from current page results (lightweight) - extract from raw merged/linked fields
  const facets = useMemo(() => {
    ...
    for (const r of rowData || []) {
      ...
      // skills: linked.Skills (split on , or ;)
    ...
    return {
      state_code: uniq(states), // still provided but the UI uses static US_STATES
      city: uniq(cities),
  ...
  const isPhoneSearchDisabled = useMemo(() => {
  ...
  const handlePhoneSearch = async () => {
    if (isPhoneSearchDisabled) return;
