 ...
 *   - City: multi-select derived from linked.Locality or merged.City
 *   - Country: multi-select from merged.Country / linked.Countries / linked.Location_Country
 *   - Job Title: multi-select from linked.Job_title
 *   - Skills: multi-select from linked.Skills
 *   - Company Name: multi-select from merged.Company or linked.Company_Name
 *   - Website / Domain: multi-select from linked.Company_Website || merged.normalized_website || merged.Web_Address
 *   - Size & Revenue: Employee Count and Total Revenue multi-select derived from merged.NumEmployees and merged.SalesVolume
 *   - Years Of Experience removed
 *
 * - Arrays are sent to backend as comma-separated values when searching.
 */
...
export default function SearchByEmail() {
  ...
  // dropdown open state for rows-per-page
  const [pageOptionsOpen, setPageOptionsOpen] = useState(false);

  // Filters shape (updated)
  ...
  // optional: close rows-per-page dropdown when clicking outside
  useEffect(() => {
  ...
  const fetchPage = async (pageNumber = 0, overrides = {}) => {
    ...
    try {
      ...
      if (emailInput && emailInput.trim() !== "") {
        mergedFilters.normalized_email = emailInput.trim().toLowerCase();
        setF((s) => ({ ...s, normalized_email: emailInput.trim().toLowerCase() }));
      }
  ...
  const handleEmailSearch = async () => {
    ...
    await fetchPage(0, { normalized_email: nv });
  };
  ...
  const isEmailSearchDisabled = useMemo(() => String(emailInput || "").trim() === "", [emailInput]);
  ...
  // Active chips (email/phone/domain + others)
  ...
  // When pageSize changes (select via dropdown) we already refetch on selection,
  // but keep a safe effect: if hasApplied and pageSize changes elsewhere, refetch page 0.
  useEffect(() => {
      ...
      <div className="h-[calc(96vh-0px)] flex">
        ...
        <main className="flex-1 px-2 pt-0 pb-6 -mt-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* email input + exact checkbox + Search button */}
            <div className="ml-3 flex items-center gap-2">
              ...
              <button
                ...
                disabled={isEmailSearchDisabled}
