...
export default function Header() {
  ...
  return (
    <header className="h-20 md:h-20 flex items-center justify-between bg-white border-b px-4 md:px-6 lg:px-8 shadow-sm z-20">
      {/* LEFT: larger logo badge + wordmark */}
      <div
      ...
      >
        ...
        <div
          className="hidden lg:block rounded-r-md"
          style={{
            width: 4,
            height: 56,
            background: "linear-gradient(180deg, var(--sidebar-from), var(--sidebar-to))",
            boxShadow: "0 2px 6px rgba(14,90,136,0.08)",
            marginRight:24,
          }}
          aria-hidden="true"
        />

        {/* BIG Badge: 72px on mobile, 80px on md+ */}
        <div
          className="flex items-center justify-center bg-white rounded-lg shadow transform transition-transform duration-150 hover:scale-105 overflow-hidden"
          style={{
            width: 72,
            height: 72,
            // padding: 10,
        ...
        >
          <div
            ...
            aria-hidden="true"
          />
          <img src={logo} alt="SalesDream" className="sr-only" draggable={false} />
        </div>

        {/* Wordmark: larger and balanced with badge */}
        <div className="flex flex-col leading-tight">
          <div className="text-xl md:text-2xl font-semibold text-sky-700 -mb-0.5">
            SalesDream
          </div>
          <div className="text-sm md:text-sm text-gray-400">Dashboard</div>
        </div>
      </div>
