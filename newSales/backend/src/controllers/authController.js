...
/**
 * Helper: ensure user is not blocked.
 * Accepts a user object (as returned from DB). Returns null if OK,
 * or an object { status, message } that can be used to short-circuit with res.
 *
 * We treat is_blocked === 1 as blocked. Use Number(...) to be robust to DB types.
 */
function ensureNotBlocked(user) {
  if (!user) return null;
  if (Number(user.is_blocked) === 1) {
    return { status: 403, message: "User is blocked" };
  }
  return null;
}
...
// Expects: { currentPassword, newPassword } in body
exports.changePassword = async (req, res) => {
  try {
    ...
    if (typeof newPassword !== "string" || newPassword.length < 6) {
      return res.status(400).json({ message: "New password must be a string and at least 6 characters" });
    }
    ...
    if (user.password_hash) {
      const match = await bcrypt.compare(String(currentPassword), String(user.password_hash));
      if (!match) return res.status(401).json({ message: "Current password is incorrect" });
    } else {
      // legacy plaintext fallback (not recommended) — compare directly
      if (!user.password || String(currentPassword) !== String(user.password)) {
    ...
    (async () => {
      try {
        ...
        const html = `
          <h3>Your password has been changed</h3>
          <p>Hello ${user.name || ""},</p>
          <p>Your account password was updated successfully. If you did not perform this change, please contact support immediately.</p>
        `;
        if (typeof sendMail === "function") {
          await sendMail({ to: user.email, subject, html });
        }
      } catch (mailErr) {
        console.warn("Failed to send change password email:", mailErr);
      }
    })();
...
// --------------------
// Login endpoints (updated to respect is_blocked)
// --------------------
...
exports.loginRequest = async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      // consistent error for bad credentials
      return res.status(401).json({ message: "Invalid email or password" });
    }

    const user = rows[0];

    // Block check: disallow login when is_blocked === 1
    ...
    // Credentials are valid — issue token immediately (no OTP)
    const token = sign(user);

    // Optionally: clear any old login OTP that may exist for this email
    try { clearOtp(`login:${email}`); } catch (e) { /* ignore */ }
...
exports.loginVerify = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      // If user exists, disallow token if blocked
      const blocked = ensureNotBlocked(user);
      if (blocked) return res.status(blocked.status).json({ message: blocked.message });
    } else {
      // create the user (not blocked by default)
